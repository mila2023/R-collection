library(openmeteo)
library(data.table)
jpeg("C:/Users/kamil/RProjects/PD2/Pogoda/lubel_RAIN_SUM.jpeg", width = 1000)
...
srtemp21 <- aggregate(woj21$daily_rain_sum  , list(woj21$month), sum)
srtemp21 <- as.double(srtemp21$x)
srtemp21

srtemp22 <- aggregate(woj22$daily_rain_sum  , list(woj22$month), sum)
srtemp22 <- as.double(srtemp22$x)
srtemp22

srtemp23 <- aggregate(woj23$daily_rain_sum  , list(woj23$month), sum)
...
par(mar=c(5,7,5,5))
barplot(counts,
        main= c(woj,"Suma opadów w zależności od miesiąca"),
        xlab="Miesiąc",ylab = "Suma opadów (mm)",cex.main=1.5,
        cex.names = 1.5, las = 2, cex.axis= 1.3,cex.lab=1.3, angle=30,
        col=c("#FFB8E0","#F50B8F","#86074F"),
        legend = rownames(counts), beside=TRUE, ylim=c(0,210), density = c(5,10,15))
...
box("plot")
legend(43.8,210,cex = 1.2 ,fill = c("#FFB8E0","#F50B8F","#86074F"),density = c(5,10,15),legend = c("rok 2021","rok 2022","rok 2023"))
dev.off()
